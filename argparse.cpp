...
class ArgParse::Impl
{
    ...
    std::tuple<Error, const Option &> lookupShortName(const std::string & name)
    {
    ...
    std::tuple<Error, const Option &> lookupLongName(const std::string & name)
    {
        int n{};
...
public:
    ...
    void add(char shortName, const char *longName, const char *description, std::function<void()> callback, const char *defaultValue)
    {
        options_.push_back({to_string(shortName), longName, {}, description, callback, {}, defaultValue, {}, {}});
    }

    void add(char shortName, const char *longName, const char *parameter, const char *description, std::function<void(const std::string &)> callback_arg, const char *defaultValue)
    {
        options_.push_back({to_string(shortName), longName, parameter, description, {}, callback_arg, defaultValue, {}, {}});
    }

    void add(char shortName, const char *longName, const char *parameter, const char *description, std::function<void(const std::string &)> callback_arg, bool required)
    {
        options_.push_back({to_string(shortName), longName, parameter, description, {}, callback_arg, {}, required, {}});
    }
    ...
    Error process(std::vector<std::string> & argv)
    {
        ...
        for (auto it = argv.begin(); it != argv.end(); ) {
            ...
            // §10 The first -- argument that is not an option-argument should be accepted as a delimiter indicating the end of options.
            ...
            if (hasPrefix(str, longDelimiter)) {
                ...
                if (hasParameter) {
                    optarg = str.substr(off + split.size());
                ...
                // §5 One or more options without option-arguments, followed by at most one option that takes an option-argument, should be accepted when grouped behind one '-' delimiter.
                if (str.empty()) {
...
void ArgParse::add(char shortName, const char *longName, const char *description, std::function<void()> callback, const char *defaultValue)
{
    pimpl->add(shortName, longName, description, callback, defaultValue);
}

void ArgParse::add(char shortName, const char *longName, const char *parameter, const char *description, std::function<void(const std::string &)> callback, const char *defaultValue)
{
    pimpl->add(shortName, longName, parameter, description, callback, defaultValue);
}

void ArgParse::add(char shortName, const char *longName, const char *parameter, const char *description, std::function<void(const std::string &)> callback, bool required)
{

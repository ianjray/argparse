...
/// @discussion Provides methods to describe the set of supported options, render help, and process command line arguments.
...
public:
    ...
    /// @brief Add an option which has an option-argument.
    /// @param shortName    Option character (or NUL if not used).
    /// @param longName     Option name (or empty string if not used).
    ...
    /// @param callback     Function called to process this option.
    /// @param defaultValue Default value (or empty string if none).
    void add(char shortName, const char *longName, const char *parameter, const char *description, std::function<void(const std::string &)> callback, const char *defaultValue = "");

    /// @brief Add an option which has an option-argument and is required.
    /// @param shortName    Option character (or NUL if not used).
    /// @param longName     Option name (or empty string if not used).
    /// @param parameter    Parameter name.
    /// @param description  Usage description.
    ...
    void add(char shortName, const char *longName, const char *parameter, const char *description, std::function<void(const std::string &)> callback, bool required);

    /// @brief Render description to stdout.
    void help() const;
    ...
    /// An error is returned if any invalid or unrecognized options are found, or if any arguments are missing, or if any required options are missing.
    /// Otherwise the callback functions are called and success is returned.
    /// @see add
